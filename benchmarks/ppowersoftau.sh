...
/usr/bin/time -v node --max-old-space-size=131072 $(which snarkjs) groth16 setup ${1} ${2} ${3}/circuit_0000.zkey 
END_1="$(date -u +%s.%N)"
...
/usr/bin/time -v snarkjs zkey contribute ${3}/circuit_0000.zkey ${3}/circuit_0001.zkey --name="TAMD2024" -v -e="ATDM2024"
END_2="$(date -u +%s.%N)"
...
/usr/bin/time -v snarkjs zkey export verificationkey ${3}/circuit_0001.zkey ${3}/verification_key.json
END_3="$(date -u +%s.%N)"
